...
| ID | Stav | Dôkaz |
|---|---|---|
| E1 | **Opravené** | Každá objednávka je vlastný kľúč s `EX` podľa `ORDER_TTL_DAYS` (30 dní), rovnako zoznam aj množiny vytlačených a hotových; počítadlo čísel má 365 dní ([`api/_store.js:30-35`, `176-193`, `217-233`](../../api/_store.js)). Žiadny kľúč s osobnými údajmi nezostáva bez expirácie. Test `E1 – objednávky sa ukladajú s expiráciou…`. Nastavenie premennej → **NEEDS MARTIN #4**. |
